...
const Message = ({message}) => {
    ...
    const handleReadClick=async()=>{
        try {
            ...
            if(res.status===200){
                ...
                console.log(read);
                console.log("Toggled to:", read);

                if(read){
                    toast.success('Marked as read');
                }
                else{
                    toast.success('Markes as new ');
                }
            }
        } catch (error) {
            console.log(error);
  ...
  <div className="relative bg-white p-4 rounded-md shadow-md border border-gray-200">
              {!isRead && (
                <div className="absolute top-2 right-2 bg-yellow-500 text-white px-2">New</div>
              )}
              ...
              <button
               onClick={handleReadClick}
                className={`mt-4 mr-3 ${isRead ? 'bg-gray-800 text-white':'bg-blue-500 text-white'}  py-1 px-3 rounded-md`}
              >
